...
const { toCents, centsDecimal, cartTotalCents, applyLiveCartPrices } = await import(`data:text/javascript;base64,${Buffer.from(moneySource).toString('base64')}`);
...
const staleCart = [{ product_id: 'prod-1', product_name: 'Chicken', sku: 'CH', quantity: 1, unit_price: 80, subtotal: 80 }];
const refreshedCart = applyLiveCartPrices(staleCart, { 'prod-1': 90 });
assert.equal(refreshedCart.changed, true);
...
assert.equal(applyLiveCartPrices(refreshedCart.items, { 'prod-1': 90 }).changed, false);
const db = new PGlite();
...
assert.equal(Number(livePriced.amount_paid), 90);
assert.equal(Number(livePriced.change_amount), 0);
assert.equal(Number((await db.query('select unit_price from public.sale_items where sale_id=$1', [livePriced.id])).rows[0].unit_price), 90);
assert.equal(cartTotalCents(refreshedCart.items) / 100, Number(livePriced.total_amount));
...
assert.equal(Number((await db.query('select unit_price from public.sale_items where sale_id=$1',[first.id])).rows[0].unit_price),80.10);
assert.equal(Number((await db.query("select quantity from public.inventory_movements where reference_type='sale' and reference_id=$1",[first.id])).rows[0].quantity),-2);
assert.equal(Number((await db.query("select quantity from public.inventory_movements where reference_type='sale' and reference_id=$1",[livePriced.id])).rows[0].quantity),-1);
await db.close();
