...
export default function CashierDashboard() {
  ...
  if (shiftQuery.isLoading && !shiftQuery.data) {
    return <LoadingState label="Checking active shift…" />;
  }
  ...
  return (
    <Screen refreshing={refreshing} onRefresh={onRefresh}>
      <ConstrainedWidth style={styles.column}>
        <PageHeader
          title={`Hello, ${profile?.full_name ?? 'Cashier'}`}
        ...
        {shiftQuery.data ? (
          <View style={styles.shiftCard}>
            ...
            <View style={styles.metricsRow}>
              <View style={styles.metric}>
                ...
                <Text style={styles.metricValue}>
                  {summaryQuery.isLoading && orders === undefined ? '…' : String(orders ?? 0)}
                </Text>
              </View>
              <View style={styles.metric}>
            ...
            {summaryQuery.error ? (
              <Text style={styles.metricError}>Unable to load shift totals. Pull to refresh.</Text>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  metricError: { color: colors.danger, fontSize: 12, lineHeight: 18 },
  warning: {
    color: '#92400E',
    backgroundColor: colors.warningSurface,
    padding: spacing.sm,
