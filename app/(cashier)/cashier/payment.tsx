...
import { cartTotalCents, centsDecimal } from '@/lib/money';
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (result.changed) {
          setPriceNotice(
            `Order total updated from ${formatMoney(result.previousTotalCents / 100)} to ${formatMoney(result.nextTotalCents / 100)}.`,
          );
        }
        setPricesReady(true);
      } catch {
